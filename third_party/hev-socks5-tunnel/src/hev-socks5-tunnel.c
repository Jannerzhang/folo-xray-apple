...
/*
 * The Hev evaluation facade supplies these hooks. They are deliberately
 * lifecycle-only callbacks: no packet, address, credential or file
 * descriptor crosses this hook. Keeping readiness at the engine boundary
 * lets the public facade make stop-before-run deterministic without probing
 * private host tunnel state.
 */
extern void folo_hev_packetflow_init_result (int result);
...
hev_socks5_tunnel_init (int tun_fd)
{
    ...
    folo_hev_packetflow_init_result (0);

    return 0;

exit:
    hev_socks5_tunnel_fini ();
    folo_hev_packetflow_init_result (-1);
    return -1;
}
