...
| Management | ABI 1.4 plus Swift/Kotlin expose routing-policy replacement/snapshots, typed SOCKS TCP/UDP, HTTP TCP, and TUN TCP/UDP connection inventory, addressable close, cumulative per-outbound accounting, bounded completed-connection event pages, and equivalent typed TUN diagnostic queues | Policy updates affect new flows only; inventory/accounting remain live/process-lifetime surfaces, completed events are capped in memory and cleared on stop; no persistent connection history |
