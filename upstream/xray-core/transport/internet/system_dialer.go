...
// ControlFunc is the narrow socket-control hook needed by the mobile build.
// It is intentionally defined here so the Folo mobile distro does not depend
// on an unrelated protocol module for this small callback type.
type ControlFunc func(network string, address string, conn syscall.RawConn) error
...
//
// xray:api:beta
func RegisterDialerController(ctl ControlFunc) error {
	if ctl == nil {
